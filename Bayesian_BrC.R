# Notes:
# 1. This is a realization of the Bayesian inference algorithm
#    to resolve [BC], [BrC], [WtC] and the K and AAE for BrC.
# 2. Parallel computations (pacakges `future` and `furrr`) are
#    implemented for speed boost
# Authorship:
# Feb 20, 2024, Kezheng LIAO in HKUST
...
prior <- function(y){
  ...
  dnorm(y["K"], K_avg, K_sd) * dexp(y["AAE"] - 1, 1/(AAE_expected - 1))
}

# Reference distribution
q0 <- function(i){
  runif(1, BrC_K * 0.75,  BrC_K * 1.25) -> K
  runif(1, 1, 4) -> AAE
